...
if _globals.PLATFORM == "linux":
    process = subprocess.run(["make", "config={}".format(_globals.BUILD_CFG.lower())]) # make uses lowercase config regardless of what is in premake5
    exit_code = process.returncode
elif _globals.PLATFORM == "windows":
    process = subprocess.run(["msbuild", "/property:Configuration={}".format(_globals.BUILD_CFG)])
    exit_code = process.returncode
